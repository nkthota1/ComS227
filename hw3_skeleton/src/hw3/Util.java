...
public class Util
{
  ...
  {
    ArrayList<Integer> runPositions = new ArrayList<>();
    for(int i=0; i < arr.length; i++)
    	if(checkRun(arr, i)) runPositions.add(i);
    return runPositions;
  }
  
  private static boolean checkRun(Jewel[] arr, int x)
	{
	  	int val = arr[x].getType();
		int left1, left2, right1, right2;
//		left1 = left2 = right1 = right2 = -1;
//		if(x-1 >= 0)
//			left1 = arr[x-1].getType();
//		if(x-2 >= 0)
//			left2 = arr[x-2].getType();
//		if(x+1 < arr.length)
//			right1 = arr[x+1].getType();
//		if(x+2 < arr.length)
//			right2 = arr[x+2].getType();
		
		left1 = (x-1 >= 0) ? arr[x-1].getType() : -1;
		left2 = (x-2 >= 0) ? arr[x-2].getType() : -1;
		right1 = (x+1 < arr.length) ? arr[x+1].getType() : -1;
		right2 = (x+2 < arr.length) ? arr[x+2].getType() : -1;
