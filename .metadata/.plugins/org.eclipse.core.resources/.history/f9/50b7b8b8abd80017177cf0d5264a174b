 ...
 * Each hand consists of an array of 
 * <em>main</em> cards and a (possibly empty) array of <em>side cards</em>.
 * The number of main cards is dependent on the type of evaluator, and
 * only the main cards are considered in determining whether a given
 * set of cards satisfies the evaluator (the <code>satisfiedBy</code> method).
 * The total number of cards in a hand (i.e., main cards plus side cards) 
 * may be greater than the required number of main cards, and this total 
 * number is always a parameter to the evaluator's constructor.  
 * <p>
 ...
 * <p>
 * For a given evaluator, the hands are also ordered.  The ordering
 * is defined by the <code>compareTo</code> method of the Hand class.  
 ...
 * <code>compareTo</code> method, which orders cards in descending order with 
 * higher cards first.  In most cases it is sufficient to sort the main cards
 * using the Card class; however, this may not work for all 
 * evaluators. It is up to the evaluator's <code>createHand</code> method to
...
public interface IEvaluator
{
  ...
  /**
   * Determines whether the given array of cards satisfies the
